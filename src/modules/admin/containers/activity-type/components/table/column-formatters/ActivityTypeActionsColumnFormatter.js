...
const ActionsColumnFormatter = (
  ...
  <div className="text-right">
    {!row.isActive && (
      ...
      >
        <ProtectedLink rule={formatExtraData.editActivityTypeRule}>
          <a
            className="btn btn-sm btn-icon btn-light btn-hover-warning btn-sm mx-1 my-1"
            onClick={() => formatExtraData.openEditActivityTypePage(row.id)}
          >
    ...
    {!row.isActive && (
      ...
      >
        <ProtectedLink rule={formatExtraData.showActivityTypeRule}>
          <a
            className="btn btn-sm btn-icon btn-light btn-hover-primary btn-sm mx-1 my-1"
            onClick={() => formatExtraData.openShowActivityTypePage(row.id)}
          >
    ...
    {!row.isActive && !row.isDeleted && (
      ...
      >
        <ProtectedLink rule={formatExtraData.disableActivityTypeRule}>
          <a
            className="btn btn-sm btn-icon btn-light btn-hover-danger btn-sm my-1 mx-1"
            onClick={() => formatExtraData.openDisableActivityTypeDialog(row.id)}
          >
    ...
    {!row.isActive && !row.isDeleted && (
      ...
      >
        <ProtectedLink rule={formatExtraData.enableActivityTypeRule}>
          <a
            className="btn btn-sm btn-icon btn-light btn-hover-success btn-sm mx-1 my-1"
            onClick={() => formatExtraData.openEnableActivityTypeDialog(row.id)}
          >
