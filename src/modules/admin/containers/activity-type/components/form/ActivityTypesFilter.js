...
import { activityTypeFields } from "./fields/filterFields"
...
const ActivityTypesFilter = ({ searchRef, clearSearchRef, intl }) => {
  ...
  const activityTypesUIContext = useActivityTypesUIContext()
  const activityTypesUIProps = useMemo(() => {
    return {
      ...activityTypesUIContext
    }
  }, [activityTypesUIContext])

  const applyFilter = (values) => {
    const newQueryParams = { ..._.pickBy({ ...activityTypesUIProps.queryParams, ...values }, _.identity) }
    if (!isEqual(newQueryParams, activityTypesUIProps.queryParams)) {
      newQueryParams.pageNumber = 1
      activityTypesUIProps.setQueryParams(newQueryParams)
    }
  }
...
export default injectIntl(ActivityTypesFilter)
