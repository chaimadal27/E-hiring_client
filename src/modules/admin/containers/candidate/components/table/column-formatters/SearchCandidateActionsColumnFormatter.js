...
const SearchActionsColumnFormatter = (
    cellContent,
    row,
    rowIndex,
    formatExtraData
            ...
            <OverlayTrigger
                overlay={
                    <Tooltip id="candidate-edit-tooltip">
                        <FormattedMessage id="CANDIDATE.AFFECTATION" />
                    </Tooltip>
                }
            >
                <ProtectedLink rule={formatExtraData.affectCandidateRule}>
                    <a
                        className="btn btn-sm btn-icon btn-light btn-hover-warning btn-sm mx-1 my-1"
                        onClick={() => formatExtraData.openAffectCandidatePage(row.id)}
                    >
...
export default SearchActionsColumnFormatter
