...
const TableOfItems = ({ field, object, history }) => {
  ...
  const { name, columns, showSearch = false, openShowPage, openValidateJobDialog, addAnotherActions, showActions = true, icon, label, size = 12 } = field
        ...
        <DisplayItem
          ...
          secondary={
            isEmpty(object) ? <TableLoader colNbr={columns.length} /> :
              <>
                {showSearch ? <SearchFilter applyFilter={({ search }) => setSearchTerm(search)} /> : ""}
